...
public class SimpleConfigManager extends ConfigManager {
    ...
    @Override
    public void addConfig(Config<?> config, Pair<Type, Object>... adapters) {
        configs.put(config, List.of(adapters));
    }

    @Override
    public void addConfig(Config<?> config, Type type, Object adaptersArray) {
        configs.put(config, List.of(new Pair<>(type, adaptersArray)));
    }

    @Override
    public void addCodecConfig(Config<?> config, Pair<Type, Codec<?>>... codecArray) {
        ...
        configs.put(config, adapters);
    }

    @Override
    public void addCodecConfig(Config<?> config, Type type, Codec<?> codec) {
        configs.put(config, List.of(new Pair<>(type, new SimpleCodecAdapter<>(codec))));
    }
